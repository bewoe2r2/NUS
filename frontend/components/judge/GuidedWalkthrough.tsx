...
export function GuidedWalkthrough({ onClose, onTabChange, onRefresh, onStepChange, initialStep }: GuidedWalkthroughProps) {
    ...
    const steps: WalkthroughStep[] = [
        ...
        },
        {
            ...
            body: "The state cards updated automatically:\n\n• Health State: STABLE (green) — all 9 biomarkers within healthy ranges\n• Risk Score: Low — composite risk weighted by clinical significance\n• 48h Crisis Probability: <10% — 2,000 simulated trajectories show safe future\n\nThe engine processed 9 signals simultaneously: glucose (25% weight), medication adherence (18%), sleep quality (10%), social engagement (10%), glucose variability (10%), daily steps (8%), carb intake (7%), heart rate variability (7%), resting heart rate (5%).\n\nIt also compensates for known clinical patterns like the Dawn Phenomenon — the early-morning glucose spike (4–8am) caused by counter-regulatory hormones. Instead of flagging a false alarm, Bewo recognizes the spike is physiological and adjusts its baseline accordingly.\n\nTry switching tabs now — on the Nurse View, the 14-day timeline shows green for days 1–5. On the Patient View, Mr. Tan sees \"You are doing well!\" with an encouraging message.\n\nLook at: The 4 state cards at the top of the Overview. All should be green. This is Mr. Tan's baseline — remember these numbers, because they're about to change.",
        ...
        },
        {
            ...
            body: "Mr. Tan is stable — but that doesn't mean the system is idle. Even in STABLE state, the agent runs these tools automatically:\n\n• Morning check-in — \"Good morning uncle! Remember to take your Metformin with breakfast\"\n• Streak celebration — \"3-day medication streak! Keep it up!\"\n• Daily challenges — personalized micro-goals (\"Walk 500 more steps than yesterday\" or \"Try one extra vegetable at dinner\") that gamify health improvement and sustain engagement day over day\n• Voucher management — maintains S$5 weekly balance as incentive\n• Food suggestions — culturally-aware (\"Try fish soup from the hawker centre — low carb, high protein\")\n• Smart scheduling — optimizes reminder timing based on when Mr. Tan actually responds\n\nEvery message is safety-checked on 6 dimensions: medical accuracy, emotional tone, hallucination, cultural sensitivity, scope, dangerous advice. Then culturally adapted to Singlish for Mr. Tan.\n\nLook at: This is the quiet power of Bewo — preventive care that feels invisible to the patient but runs continuously in the background.",
        ...
        // ===============================================
        {
            ...
            body: "Now that we've seen Mr. Tan stable, let's see what happens when things change. His daughter is travelling. He's eating out more. Skipping his walks.\n\nThe data tells the story:\n\n• Glucose creeping up: 6.0 → 7.5 → 9.0 mmol/L\n• Medication adherence dropping: 95% → 80% → 65%\n• Steps declining: 5,500 → 3,500 → 2,000/day\n• HRV falling: 44ms → 35ms → 26ms (autonomic stress)\n• Sleep quality dropping: 7.5 → 6.0 → 5.0\n• Social interactions: decreasing\n\nIndividually, none of these trigger an alert. Mr. Tan's glucose is \"borderline.\" His step count is \"a bit low.\" Any single metric looks okay.\n\nBut the HMM sees all 9 features TOGETHER — and detects a pattern that no threshold-based system would catch. Combined-risk rules amplify this: when glucose rises AND medication adherence drops AND activity declines simultaneously, the risk multiplier compounds beyond what any single metric would trigger. These multi-signal rules are why Bewo catches crises that single-threshold monitors miss entirely.\n\nLook at: After clicking, watch the state cards shift from green to amber. The sidebar console shows the HMM processing in real time.",
        ...
        },
        {
            ...
            body: "Mr. Tan’s daughter, Mei Ling, just received a push notification: \"Dad’s health is declining slightly. He missed his evening Metformin twice this week.\"\n\nShe didn’t have to call the clinic. She didn’t have to guess. One tap: \"I’ll check in on Dad tonight.\"\n\nClick \"Load Caregiver Dashboard\" below to pull Mei Ling’s live dashboard from the API. You’ll see:\n\n• Active alerts — colour-coded by urgency (green/amber/red)\n• Burden score (0–100) — how overwhelmed Mei Ling is right now\n• Response streaks — consecutive days she’s responded to alerts\n• One-tap response buttons — \"I’ll check in\", \"Call me\", \"Noted\"\n\nBewo uses a 3-tier escalation system that matches alert severity to communication channel:\n\n• Info tier (stable drift) → in-app push notification\n• Warning tier (pattern shift) → SMS with action link\n• Critical tier (crisis detected) → SMS + automated phone call\n\nBut here’s what no competitor does: Bewo tracks caregiver burden. If Mei Ling’s burden exceeds 70, the system auto-switches to daily digest mode — batching non-urgent updates into one evening summary. This prevents alert fatigue. Overwhelmed caregivers stop responding entirely. Bewo keeps Mei Ling engaged by respecting her capacity.\n\nLook at: The Caregiver tab is now active — notice the alert cards, burden gauge, and one-tap response buttons. You can also open /caregiver in a new browser tab to see the standalone caregiver mobile experience. On the Intelligence tab, find the Caregiver Burden gauge and response-streak chart.",
        ...
        },
        {
            ...
            subtitle: "3 stakeholders see the same event differently — click each tab to compare",
            body: "Mr. Tan's blood glucose has been drifting upward for 3 days and he missed evening Metformin twice. One clinical event — three completely different experiences:\n\nPatient sees: Warm Singlish encouragement — \"Uncle, your sugar a bit high today lah. Take extra care, can?\" A gentle yellow insight card, no medical jargon, no panic. His voucher balance shows a small deduction for the missed dose. Tone: supportive, culturally familiar.\n\nNurse sees: SBAR clinical report — Situation: BG trending +15% over 72h. Background: Type 2 DM, Metformin 500mg BD. Assessment: HMM state shifted STABLE→WARNING. Recommendation: Review medication adherence, consider telehealth follow-up. Mr. Tan is ranked SOON in the triage queue with amber urgency.\n\nCaregiver sees: \"Your father's health needs a check-in\" — plain language, no clinical codes. Mei Ling gets one-tap response buttons: \"I'll visit tonight\", \"Call me to discuss\", \"Noted, keep monitoring\". Her burden score stays visible so the system knows not to over-alert her.\n\nThis is the core insight: same intelligence engine, three purpose-built interfaces. No one sees information meant for someone else.\n\nLook at: Click the Patient, Nurse, and Caregiver tabs in the tab bar above the main content. On the Patient tab, find the yellow insight card with Singlish text. On the Nurse tab, find the SBAR report and triage ranking. On the Caregiver tab, find the alert card with one-tap response buttons. Notice how the same health decline is communicated in completely different tones and formats for each stakeholder.",
            insight: "Most health AI shows everyone the same dashboard. Bewo gives clinicians SBAR reports with urgency triage, patients warm Singlish encouragement, and caregivers plain-language status with one-tap response buttons. Same intelligence engine, three purpose-built experiences.",
            tab: "overview",
            icon: <Shield size={20} />,
        ...
        // ===============================================
        {
            ...
            body: "The warning interventions slowed the decline — but Mr. Tan's condition continues to worsen. This is what happens when a patient lives alone and a caregiver can't intervene fast enough.\n\n• Glucose: 9.5 → 12.0 → 15.0+ mmol/L\n• Medication adherence: collapsed to ~30%\n• Steps: barely 400/day\n• Heart rate variability: crashed to <15ms (severe stress)\n• Sleep: 2.0/10\n• Social engagement: withdrawn\n\nThis is the exact pattern that precedes an ER visit. Without Bewo, the next stop is a $8,800 hospital admission that could have been prevented.\n\nLook at: After clicking, watch the state cards turn red. The sidebar console shows all 5 Diamond layers escalating simultaneously. This is maximum alert.",
        ...
        },
        {
            ...
            body: "Mr. Tan is safe. He's at home, not in the ER. The dashboard confirms it:\n\n• Health State: STABLE (green) — recovered\n• Risk Score: ~22% (down from 95%)\n• 48h Crisis Probability: Low — safe trajectory confirmed\n• Triage: IMMEDIATE → STABLE\n\nCheck the Nurse View — the timeline shows the full arc: green → amber → red → green. The system learned that Mr. Tan responds best to combined medication reminders + caregiver involvement + scheduled appointments.\n\nCheck the Patient View — green insight card: \"You are doing well!\" Streak counter reset. Voucher balance intact.\n\nThe math: 1 prevented ER visit = $8,800 saved. Bewo costs $0.40/patient/month. At scale, 87% gross margin.\n\nLook at: State cards are green again. Compare the Nurse View timeline — the full journey from green to amber to red to green tells the complete story in one glance.",
